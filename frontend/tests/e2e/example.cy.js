describe('Shopping Lists', () => {
  ...
  it('deletes a list', () => {
    ...
    cy.contains('Zu löschende Liste').parents('.bg-white').find('button[title="Liste löschen"]').click()
  ...
  it('shows empty state after last list is deleted', () => {
    ...
    cy.contains('Einzige Liste').parents('.bg-white').find('button[title="Liste löschen"]').click()
    cy.contains('Liste löschen?').should('be.visible')
    cy.contains('button', 'Löschen').click()
    cy.contains('Einzige Liste').should('not.exist')
  })
})